          ...
          return `
                <div style="background:#f9f9f9; padding:1.5rem; margin-bottom:1rem; border-left:4px solid ${stColor}; border-radius:4px;">
                    ...
                    <div style="display:flex; justify-content:space-between; align-items:center;">
                        <p style="font-size:1.3rem; color:${stColor}; font-weight:bold; margin-bottom:0;">Status: ${st}</p>
                        ${st === 'Scheduled' ? `<button onclick="cancelAppointment(${appt.id})" style="background:#dc3545; color:white; border:none; padding:0.5rem 1rem; border-radius:4px; cursor:pointer;">Cancel</button>` : ''}
                    </div>
                </div>
            `;
        })
...
window.cancelAppointment = async function(id) {
    ...
    const email = localStorage.getItem("userEmail");
    if (!email) {
        alert("User email not found. Please relogin.");
        return;
    }

    try {
        const res = await fetch(`${API_BASE}/api/cancel-appointment`, {
             method: 'POST',
             headers: { 
